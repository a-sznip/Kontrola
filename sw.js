const STATIC_CACHE_NAME = 'kontrole-static-v13'; // Zmiana wersji, aby wymusić aktualizację
const DYNAMIC_CACHE_NAME = 'mzmgo-dynamic-v2';
...
self.addEventListener('fetch', event => {
  // *** KLUCZOWA POPRAWKA ***
  // Ignoruj wszystkie żądania, które nie są typu GET.
  // To zapobiega ingerencji w proces logowania (który używa POST).
      ...
      caches.match(event.request)
        .then(response => {
          return response || fetch(event.request).then(networkResponse => {
            // Dla bezpieczeństwa, cachujemy tylko zasoby z listy
