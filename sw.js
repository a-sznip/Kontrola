                    ...
                    <div>
                        ...
                        <div class="p-3 bg-gray-50 rounded-lg border text-sm">
                            ...
                            <p class="mt-2"><strong>Otwórz w mapie:</strong> <a id="open-map-link" href="#" target="_blank" class="text-blue-600 hover:underline">OpenStreetMap</a></p>
                             <p class="mt-2"><strong>Sprawdź w systemie SIP:</strong> <a id="geoportal-link" href="#" target="_blank" class="text-blue-600 hover:underline">Geoportal.gov.pl</a></p>
                        </div>
                    </div>
                    <div>
                        <label for="identification-plot-number" class="block text-gray-700 font-medium mb-1">Numer Działki (z Geoportalu)</label>
                        <input type="text" id="identification-plot-number" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="Wklej numer działki skopiowany z Geoportalu">
                    </div>
                    <div>
    ...
    <script>
        ...
        async function generateProtocolPDF() {
            ...
            try {
                ...
                const protocolDate = control.protocol.date || control.date;
                const sanitizedAddress = `${control.city}_${control.street}_${control.houseNumber}`.replace(/[\s/\\?%*:|"<>]/g, '-');
                const finalFileName = `Protokol_${protocolDate}_${sanitizedAddress}.pdf`;
        ...
        async function generateJointNotePDF() {
            ...
            try {
                ...
                const finalFileName = `Notatka_Sluzbowa_z_${dateFilter}.pdf`;
                const blob = pdf.output('blob');
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
        ...
        function openIdentificationModal(id = null) {
            ...
            if (id) {
                ...
                document.getElementById('geoportal-link').href = `https://mapy.geoportal.gov.pl/imap/Imgp_2.html?identify_point=${item.longitude},${item.latitude}`;
        ...
        function displayIdentificationPreview() {
            ...
            document.getElementById('geoportal-link').href = `https://mapy.geoportal.gov.pl/imap/Imgp_2.html?identify_point=${lon},${lat}`;
        ...
        // --- PDF GENERATION FOR IDENTIFICATION ---
        function getMapImageAsDataUrl(latitude, longitude) {
            return new Promise((resolve) => {
                const bboxDelta = 0.001; // Controls the zoom level. Smaller is more zoomed in.
                const west = parseFloat(longitude) - bboxDelta;
                const south = parseFloat(latitude) - (bboxDelta * 0.75); // Adjust for 800x600 aspect ratio
                ...
                mapImg.onload = () => {
                    ...
                    markerImg.onerror = () => {
                        ...
                        resolve(canvas.toDataURL('image/jpeg')); // Zwróć mapę nawet bez pinezki
                    };
        ...
        async function generateIdentificationPDF() {
            ...
            try {
                ...
                const finalFileName = `Identyfikacja_${date}_${sanitizedPlot}.pdf`;
        ...
        function exportData() {
            ...
            a.href = url;
            a.download = `kopia_zapasowa_${date}.json`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            /*
            // Rejestracja Service Workera została tymczasowo wyłączona w celu naprawy problemu z logowaniem.
